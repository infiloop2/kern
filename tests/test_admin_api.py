...
class AdminRouteTableTests(unittest.TestCase):
    """Dispatch behavior of the admin API route table. These cover matching,
    ordering, and the checks the table declares, so they patch the handlers
    and need no database."""

    def _resolve(self, method: str, path: str) -> admin_api._Route | None:
        for entry in admin_api._ROUTES:
            matched, _ = admin_api._route_matches(entry, method, path)
            if matched:
                return entry
        return None
    ...
    def _route(
        ...
        return admin_api.route(
            ...
            query or {},
            body,
            principal=admin_api.OperatorPrincipal("session-hash"),
        )

    def test_an_unknown_path_is_not_found(self) -> None:
        with self.assertRaises(admin_api.ApiError) as error:
            self._route("GET", "/v1/nonexistent")

        self.assertEqual(error.exception.status, HTTPStatus.NOT_FOUND)
        self.assertEqual(error.exception.message, "route not found")

    def test_a_known_path_with_no_entry_for_the_method_is_not_found(self) -> None:
        # The table never answers 405: an unrouted method is indistinguishable
        # from an unrouted path to an unauthenticated prober.
        with self.assertRaises(admin_api.ApiError) as error:
            self._route("DELETE", "/v1/health")

        self.assertEqual(error.exception.status, HTTPStatus.NOT_FOUND)
        self.assertEqual(error.exception.message, "route not found")

    def test_a_numeric_event_id_is_matched_before_the_tools_subtree(self) -> None:
        with (
            patch.object(admin_api.state, "tool_event", return_value={"id": 12}) as tool_event,
            patch.object(admin_api.tools_admin_api, "tools_route") as tools_route,
        ):
            response = self._route("GET", "/v1/tools/events/12")

        self.assertEqual(response, {"event": {"id": 12}})
        tool_event.assert_called_once_with(12)
        tools_route.assert_not_called()

    def test_a_non_numeric_event_id_falls_through_to_the_tools_subtree(self) -> None:
        with patch.object(
            admin_api.tools_admin_api, "tools_route", return_value={"tools": []}
        ) as tools_route:
            response = self._route("GET", "/v1/tools/events/not-a-number")

        self.assertEqual(response, {"tools": []})
        tools_route.assert_called_once_with("GET", "/v1/tools/events/not-a-number", None)

    def test_every_proxied_workspace_subtree_reaches_the_workspace_proxy(self) -> None:
        for subtree in admin_api._WORKSPACE_PROXY_SUBTREES:
            for path in (f"/v1/workspace/{subtree}", f"/v1/workspace/{subtree}/thread-1"):
                with self.subTest(path=path):
                    with patch.object(
                        admin_api.workspace_proxy, "route_request", return_value={"ok": True}
                    ) as proxied:
                        response = self._route("GET", path)

                    self.assertEqual(response, {"ok": True})
                    proxied.assert_called_once_with("GET", path, {}, None)

    def test_a_workspace_principal_cannot_reach_a_proxied_subtree(self) -> None:
        # The shared allowlist already denies these paths; the route entry's
        # own operator-only flag is the defense in depth behind it.
        with (
            patch.object(
                admin_api.workspace_admin_api,
                "is_allowed_workspace_admin_route",
                return_value=True,
            ),
            patch.object(admin_api.workspace_proxy, "route_request") as proxied,
        ...
        self.assertEqual(error.exception.message, "Workspace service route is not allowed")
        proxied.assert_not_called()

    def test_every_workspace_allowlisted_route_has_a_table_entry(self) -> None:
        for method, pattern in workspace_admin_api.WORKSPACE_ALLOWED_ADMIN_ROUTES:
            path = "/".join(
                "thread-1" if part.startswith(":") else part for part in pattern.split("/")
            )
            with self.subTest(method=method, path=path):
                self.assertIsNotNone(self._resolve(method, path))

    def test_an_unsupported_query_parameter_is_rejected_before_the_handler(self) -> None:
        with (
            patch.object(admin_api, "list_threads") as list_threads,
            self.assertRaises(admin_api.ApiError) as error,
        ):
            self._route("GET", "/v1/threads", {"nope": ["1"]})

        self.assertEqual(error.exception.status, HTTPStatus.BAD_REQUEST)
        self.assertEqual(error.exception.message, "unsupported thread list query parameter: nope")
        list_threads.assert_not_called()

    def test_a_malformed_pending_push_action_is_not_found(self) -> None:
        with self.assertRaises(admin_api.ApiError) as error:
            self._route("POST", "/v1/network-tools/github-pending-pushes/push-1/maybe")
...
class AgentProcessSnapshotTests(unittest.TestCase):
    def test_agent_processes_reads_descendant_cgroup_proc_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            with (
                patch("host.runtime.core.host_metrics.AGENT_CGROUP_ROOT", root / "cgroup"),
                patch("host.runtime.core.host_metrics.PROC_ROOT", proc),
            ):
...
class AdminApiIntegrationTests(unittest.TestCase):
    ...
    def test_filesystem_metrics_reports_root_and_data_mounts(self) -> None:
        ...
        with patch("host.runtime.core.host_metrics.shutil.disk_usage", side_effect=fake_disk_usage):
    ...
    def test_health_never_spawns_codex(self) -> None:
        ...
        with patch(
            "host.runtime.agent_runtime.orchestrator.codex_app_server.account_status",
    ...
    def test_disabled_provider_runtime_is_deactivated_without_cli_check(self) -> None:
        ...
        with patch(
            "host.runtime.agent_runtime.orchestrator.claude_code.account_status",
    ...
    def test_agent_file_routes_use_sudo_helper(self) -> None:
        ...
        with patch("host.runtime.core.root_helpers.subprocess.run", side_effect=fake_run):
    ...
    def test_agent_file_helper_errors_map_to_http_status(self) -> None:
        ...
        with patch("host.runtime.core.root_helpers.subprocess.run", side_effect=missing):
            with self.assertRaises(urllib.error.HTTPError) as error:
    ...
    def test_agent_file_helper_permission_error_during_timeout_returns_504(self) -> None:
        with patch("host.runtime.core.root_helpers.subprocess.run", side_effect=PermissionError("kill denied")):
            with self.assertRaises(urllib.error.HTTPError) as error:
    ...
    def test_reboot_helper_swallows_unkillable_timeout(self) -> None:
        ...
        for effect in (subprocess.TimeoutExpired(cmd="reboot-host", timeout=10), PermissionError("not permitted")):
            with patch("host.runtime.core.root_helpers.subprocess.run", side_effect=effect):
    ...
    def test_reboot_helper_failure_returns_500(self) -> None:
        ...
        with patch("host.runtime.core.root_helpers.subprocess.run", return_value=failed):
            with self.assertRaises(admin_api.ApiError) as error:
    ...
    def test_login_completion_clears_device_login_record(self) -> None:
        ...
        with (
            patch(
                "host.runtime.agent_runtime.orchestrator.codex_app_server.read_completed_device_login_account_id",
    ...
    def test_active_claude_runtime_refresh_records_rotated_token(self) -> None:
        ...
        with (
            ...
            patch(
                "host.runtime.agent_runtime.orchestrator.claude_code.read_attested_identity",
    ...
    def test_connect_bedrock_credentials_validates_and_refreshes(self) -> None:
        ...
        with (
            patch(
                "host.runtime.agent_runtime.orchestrator.replace_and_validate_bedrock_credentials",
                return_value=("active", None),
            ) as replace,
            patch(
                "host.runtime.agent_runtime.orchestrator.refresh_runtime_status",
                return_value="active",
            ) as refresh_status,
        ):
    ...
    def test_disconnect_bedrock_credentials_clears_shared_credential(self) -> None:
        ...
        with patch(
            "host.runtime.agent_runtime.orchestrator._stop_runtime_processes"
    ...
    def test_grok_oauth_start_retries_after_clearing_an_unanchored_credential(self) -> None:
        ...
        with (
            patch(
                "host.runtime.admin_api.runtime_accounts.grok_agent.start_device_login",
                side_effect=[
            ...
            patch(
                "host.runtime.admin_api.runtime_accounts._clear_local_agent_auth"
            ) as clear,
        ):
    ...
    def test_reboot_uses_privileged_helper(self) -> None:
        ...
        with patch("host.runtime.core.root_helpers.subprocess.run", return_value=succeeded) as run:
...
class ToolRoutesTests(unittest.TestCase):
    ...
    def test_listing_requires_auth_and_reports_manifest_state(self) -> None:
        ...
        self.assertEqual(gmail["connected_accounts"], [])
    ...
    def test_connect_flow_routing_and_gating(self) -> None:
        ...
        self.assertTrue(body["connection_id"].startswith("connection_"))
        self.assertRegex(body["connection_id"], r"^[a-z][a-z0-9_-]{0,63}$")
        connection_id = body["connection_id"]

        with self.assertRaises(urllib.error.HTTPError) as error:
            self.request("POST", "/v1/tools/gmail/oauth_connect/disconnect")
        self.assertEqual(error.exception.code, 400)
        ...
        status, body = self.request(
            "POST", "/v1/tools/gmail/oauth_connect/disconnect", {"connection_id": connection_id}
        )
        self.assertEqual(
            body, {"tool_id": "gmail", "connection_id": connection_id, "connected": False}
        )
    ...
    def test_multiple_oauth_accounts_are_listed_and_disconnected_independently(self) -> None:
        ...
        with patch.dict(admin_api.tools_host.BUNDLED_TOOLS, {"fake_notes": FakeTool()}):
            ...
            for _ in range(2):
                start = self.request(
                    "POST",
                    "/v1/tools/fake_notes/oauth_connect/start",
                    {"redirect_uri": "http://localhost:7443/oauth/callback"},
                )[1]
                connection_id = start["connection_id"]
                connected_ids.append(connection_id)
                # FakeTool always returns the same provider account, so give
                # the second connection a distinct test identity at the
                # storage boundary after exercising the callback routing.
                if len(connected_ids) == 1:
                    self.request(
                        "POST",
                        "/v1/tools/fake_notes/oauth_connect/complete",
                        {
                            "code": "code",
                            "state": "state-1",
                            "redirect_uri": "http://localhost:7443/oauth/callback",
            ...
            listed = self.tool_entry(self.request("GET", "/v1/tools")[1], "fake_notes")
            self.assertEqual(
                {item["connection_id"] for item in listed["connected_accounts"]},
                set(connected_ids),
            )
            self.request(
                "POST",
                "/v1/tools/fake_notes/oauth_connect/disconnect",
                {"connection_id": connected_ids[1]},
            )
            remaining = self.tool_entry(self.request("GET", "/v1/tools")[1], "fake_notes")
            self.assertEqual(
                [item["connection_id"] for item in remaining["connected_accounts"]],
                [connected_ids[0]],
            )
    ...
    def test_approval_decisions(self) -> None:
        ...
        with patch.dict(admin_api.tools_host.BUNDLED_TOOLS, {"fake_notes": FakeTool()}):
            ...
            self.request("POST", "/v1/tools/fake_notes/enable")
            admin_api.tools_host.HostCredentials(
                "fake_notes",
                admin_api.tools_host.ConnectionScope("connection_test", None),
            ).save(
                {
                    "account": {
                        "id": "acct-1",
    ...
    def test_host_diagnostics_endpoint_is_read_only_filterable_and_lazy(self) -> None:
        event = {
            ...
            "traceback": 'File "host/runtime/agent_runtime/orchestrator.py", line 1, in execute',
