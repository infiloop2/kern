...
host/
  ...
  runtime/                  # one package per service process; each socket has
    ...
    agent_app/              # kern-agent-app: agent app_api socket
...
| `host/runtime/agent_shim/mcp_shim.py` | `kern-agent`, spawned by each harness | Aggregates the peer-authenticated tools, network-introspection, and app sockets over one stdio MCP server. |
