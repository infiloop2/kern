...
credits. OpenAI Image Generation is the one media check that must spend: the
tool exposes no read-only endpoint, so it renders one image with the cheapest
model at the lowest quality (about a cent) and deletes the saved file
afterwards, which is also the only end-to-end proof that generated bytes reach
the agent workspace.
