...
| `proxy_provider_pins` | The proxy-readable switch for which OAuth account is currently allowed to make requests. It contains only `provider` and `account_id`. The admin service can clear this row automatically when a runtime is inactive without changing the durable, user-controlled anchor in `provider_accounts`; only an explicit user reset clears that anchor. OpenAI requests expose and are checked directly against the allowed identity. Claude bearer tokens expose no account field, so the proxy provider-attests each distinct token against it and caches successful token hashes only in memory. Hermes has no row because the proxy reads its validated Bedrock credential directly. |
