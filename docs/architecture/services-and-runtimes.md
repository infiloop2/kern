...
| `kern-workspace.service` | `kern-workspace` | One Chat, Web Apps, global Memory, and Schedules backend. The admin API uses `/run/kern-workspace/browser.sock`; agents use the separate `/run/kern-workspace/agent.sock`. Both sockets authenticate peer uid. Its Postgres role has explicit DML-only access to the Workspace tables in `public` and no egress. |
