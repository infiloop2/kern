...
under [Access Enforcement](#access-enforcement)) and publishing to GitHub Pages.
Dispatching an existing GitHub Actions workflow is treated as a scoped write:
it is allowed only for a configured write repository. Other workflow and run
administration remains denied.
...
| Workflows and their triggers | `GET /repos/{o}/{r}/contents/.github/workflows`, then a plain substring search of each file for the dangerous trigger names (false positives over-warn, which is fine) | Any workflow: an agent push or PR can execute agent-written code with whatever secrets and network that repo grants. `pull_request_target` adds a warning to review the workflow carefully and ensure it does not run untrusted pull request code with base-repository privileges. |
