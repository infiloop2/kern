...
Each workspace owns one coherently revisioned generated UI bundle and JSON
document, a sparse restorable revision history, and one fixed agent thread. Browser calls use the
authenticated admin proxy, while agent calls use the service's
peer-authenticated Unix socket.
...
the workspace row is locked. The shared token prevents an interface from
being published against data that changed after the agent read it.
"""
...
def _insert_revision(
    ...
    *,
    ...
    cur.execute(
        ...
        " data_json, created_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
        (
            app_id, revision, actor, kind, restored_from, html, css,
            javascript, data_json, now,
        ),
    )
...
def restore_revision(app_id: str, revision: int) -> dict[str, Any]:
    ...
    with db.transaction() as cur:
        ...
        cur.execute(
            "SELECT html, css, javascript, data_json FROM web_app_revisions"
