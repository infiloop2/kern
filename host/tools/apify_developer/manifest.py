...
MANIFEST = ToolManifest(
    ...
    actions=protect_inputs((
        ...
        write("set_monetization", "Replace pay-per-event pricing for an owned Actor immediately after approval. Sends one pricing record without preserving previous periods. Provider rules may reject immediate changes; no scheduling or automatic retry.",
              {"actor_id": ID,
    ...
    technical_details=(PARAM_GUARD_TECHNICAL_DETAIL,
        ...
        "Pricing approvals contain the replacement record; execution adds creation/effective timestamps for immediate application. Existing records are not sent; the new record includes Apify's documented standard 20% share. Provider notice and seller requirements may reject immediate application; no scheduling fallback is attempted.",
