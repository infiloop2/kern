...
MANIFEST = ToolManifest(
    ...
    actions=protect_inputs((
        ...
        ActionSpec(id="export_results", description="Download up to 100 default-dataset records from an owned run as JSON; maximum 2 MiB.",
        ...
        ActionSpec(id="run_actor", approval="operator", description="Run an exact successful build of your own Actor after approval: at most $0.50, 120 seconds and 1024 MiB, with limited permissions.",
    ...
    protections=(PARAM_GUARD_PROTECTION,
        "Code uploads, builds, each paid test run and public releases require approval bound to the configured account and target Actor.",
    ...
    technical_details=(PARAM_GUARD_TECHNICAL_DETAIL,
        ...
        "Build approvals fingerprint source and settings; each build gets a unique candidate tag. Publication pins an immutable successful build and rechecks listing settings.",
    ...
    setup_steps=(
        ...
        SetupStep(title="Connect Apify Developer", description="Enter the token here and enable the integration. Its configuration is independent of Apify Business Data. Pricing and full seller Insights remain in Apify Console.", show_config=True),
    ),
    data_summary=DataSummary(cards=(
        DataSummaryCard(title="What leaves this host", description="Search phrases, lookup ids, Actor input, and approved source files, documentation and listing text go to Apify. The configured API token authenticates requests; do not include secrets in source or inputs."),
