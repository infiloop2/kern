...
def write(action, description, fields, required):
    policies = {
        "set_monetization": "Before approval, checks account ownership. After approval, rechecks ownership and appends the approved pay-per-event record to existing pricing history to api.apify.com with creation and effective timestamps set to execution time. Existing pricing records are read again, recursively parameter-guarded and included unchanged before the new record; history is not put in the approval payload. Existing scheduled pricing must be resolved in Console first. Event titles, descriptions, prices, primary and one-time flags and minimum allowed run budget affect customer billing. Includes the documented standard 20% Apify revenue share in the approved new record. Does not publish, start runs, change usage billing, suppress notifications or configure payouts. Reads back the current pricing. Provider notice and seller requirements may reject the request; the tool never automatically schedules or retries it.",
...
MANIFEST = ToolManifest(
    ...
    actions=protect_inputs((
        ...
        read("get_monetization", "Read the current and next scheduled pricing periods and the current icon URL of an owned Actor. Missing pricing is not proof monetization is disabled.", {"actor_id": ID},
             o.obj({"actor_id": ID, "pricing_json": o.text("JSON array of the current and next scheduled provider pricing records, up to 24 KiB; empty if unavailable, credentials redacted. No earnings or paid-billing proof."),
                    "picture_url": o.text("Current provider icon URL, or empty if unavailable. Not fetched; no icon upload API is exposed.")}), ["actor_id"]),
        write("set_monetization", "Replace pay-per-event pricing for an owned Actor immediately after approval. Preserves existing pricing history and appends one approved record; no scheduled pricing. Provider rules may reject immediate changes; no scheduling or automatic retry.",
              {"actor_id": ID,
              ...
              ["actor_id", "minimum_run_budget_usd", "events"]),
    ...
    technical_details=(PARAM_GUARD_TECHNICAL_DETAIL,
        ...
        "Pricing approvals contain only the new record; execution adds creation/effective timestamps for immediate application and sends freshly read, recursively parameter-guarded existing history unchanged alongside it. History over 64 KiB or scheduled pricing requires Console review. The new record includes Apify's documented standard 20% share. Provider notice and seller requirements may reject immediate application; no scheduling fallback is attempted.",
