    ...
    try:
        ...
        with OperationLock("lima", config.agent_name):
            ...
            if mode == "start":
                ...
                _check_stored_definition(config, inventory.instance)
                if initial_state == "Stopped":
                    _limactl(config, "start", "--tty=false", instance_name, timeout=_START_TIMEOUT_SECONDS)
                instance = _launched_instance(config, instance_name)
                final_state = "running"
            else:
                if initial_state not in {"Running", "Stopped"}:
                ...
                if initial_state == "Running":
                    _limactl(config, "stop", instance_name, timeout=_STOP_TIMEOUT_SECONDS)
                refreshed = _inspect(config).instance
